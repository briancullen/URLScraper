...
def checkURLs (urlsToCheck, urlsChecked, linkCount):
	lock.acquire()
	while len(urlsToCheck) > 0:
		currentURL = urlsToCheck.pop()
		urlsChecked.add(currentURL)
		#print("Checking page (" + currentURL + ") -", threading.get_ident())
		lock.release()
...
try:
	threads = [threading.Thread(None, checkURLs, None, (urlsToCheck, urlsChecked, linkCount))]
	threads[0].start()
	time.sleep(1)

	for index in range(5):
		thread = threading.Thread(None, checkURLs, None, (urlsToCheck, urlsChecked, linkCount))
		thread.start()
